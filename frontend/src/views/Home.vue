      ...
      <div v-else class="movie-grid">
        <div class="movie-card" v-for="movie in movieList" :key="movie.id" @click="goToDetail(movie.id)">
          <div class="movie-poster">
            <img 
              :src="getPosterUrl(movie.poster, movie.id)" 
              :alt="movie.title"
...
export default {
  ...
  methods: {
    getPosterUrl(poster, id) {
      if (poster && poster.startsWith('http')) {
        return poster;
      }
      return `https://picsum.photos/220/300?random=${id}`;
    },
    ...
    async loadMovies() {
      ...
      try {
        ...
        const res = await movieApi.getAllMovies(params);
        console.log('全部电影响应:', res);
